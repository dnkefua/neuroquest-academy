...
const C1 = '#F59E0B', C2 = '#EF4444';

export default function QuizScene() {
  const { currentQuestion, questions, currentQuestId, score, clueUsed, answerQuestion, openClue, nextQuestion, setScene } = useEnglishStore();
  ...
  if (!question) return (
    <div className="h-dvh flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #1f1400, #2d1f00)' }}>
  ...
  return (
    <div className="h-dvh overflow-hidden flex flex-col"
      style={{ background: 'linear-gradient(160deg, #1f1400 0%, #2d1f00 60%, #1a1000 100%)' }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={currentQuestion}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-3 pt-2.5 pb-1">
            <ConceptAnimation subject="english" questTitle={quest?.title} color1={C1} color2={C2} />
