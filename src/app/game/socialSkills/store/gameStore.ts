...
export type SocialSkillsScene = 'QUEST_MAP' | 'MISSION_BRIEFING' | 'QUIZ' | 'VICTORY';

export interface SocialSkillsQuestion {
  id: number;
  narrative: string;
  question: string;
  options: string[];
...
interface SocialSkillsState {
  scene: SocialSkillsScene;
  currentGrade: number;
  currentQuestId: string;
  studentName: string;
  currentQuestion: number;
  questions: SocialSkillsQuestion[];
  score: number;
  clueUsed: boolean[];
  xpEarned: number;
  setScene: (scene: SocialSkillsScene) => void;
...
function getQuestsForGrade(grade: number): GameQuest[] {
  return getGameQuests(grade, 'socialSkills' as CurriculumSubject);
}

// Convert GameQuestion to SocialSkillsQuestion format
function toSocialSkillsQuestion(q: GameQuest['questions'][0], index: number): SocialSkillsQuestion {
  return {
    id: index + 1,
...
export const useSocialSkillsStore = create<SocialSkillsState>((set, get) => ({
  scene: 'QUEST_MAP',
  ...
  setGrade: (grade) => {
    ...
    set({
      ...
      questions: firstQuest.questions.map((q, i) => toSocialSkillsQuestion(q, i)),
      currentQuestion: 0,
      score: 0,
  ...
  loadQuest: (questId, grade) => {
    ...
    set({
      ...
      questions: quest.questions.map((q, i) => toSocialSkillsQuestion(q, i)),
      currentQuestion: 0,
      score: 0,
  ...
  nextQuestion: () => set((s) => {
    ...
    if (next >= s.questions.length) return { scene: 'VICTORY' as SocialSkillsScene };
