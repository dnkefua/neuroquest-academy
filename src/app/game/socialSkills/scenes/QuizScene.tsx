...
export default function QuizScene() {
  ...
  const handleClue = () => {
    if (walletCoins >= question.clue.cost && !clueUsed[currentQuestion]) {
      spendCoins(question.clue.cost, 'social_skills_clue');
    ...
    <div className="min-h-screen flex flex-col"
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10"
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-pink-400">💰 {walletCoins}</span>
        </div>
        <button onClick={() => setTtsOn(gameTTS.toggle())}
          className="w-9 h-9 rounded-full flex items-center justify-center text-base"
          style={{ background: 'rgba(255,255,255,0.08)', border: `1px solid ${ttsOn ? 'rgba(236,72,153,0.5)' : 'rgba(255,255,255,0.15)'}` }}>
          {ttsOn ? '🔊' : '🔇'}
        </button>
      </div>
          ...
          <motion.div
            ...
            {!showResult && !showClue && !clueUsed[currentQuestion] && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 text-center">
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-sm font-bold text-pink-300 border border-pink-300/30 hover:bg-pink-300/10 transition-all disabled:opacity-50"
            ...
            {showClue && (
              <motion.div
                ...
                <p className="font-bold text-pink-300 text-sm mb-1">💡 {question.clue.title}</p>
