...
import { useSocialSkillsStore, getQuestById } from '../store/gameStore';
...
export default function MissionBriefing() {
  const { currentQuestId, setScene } = useSocialSkillsStore();
  ...
  return (
    <div className="min-h-screen flex flex-col"
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10"
        ...
        <span className="text-xs text-gray-400">Grade {grade} Social Skills</span>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <motion.div
          ...
          <motion.button
            ...
            style={{ background: `linear-gradient(135deg, ${quest.color}, #EC4899)` }}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}>
            <span className="relative z-10">💜 Start Social Skills Quest</span>
          </motion.button>
        </motion.div>
      </div>
    </div>
