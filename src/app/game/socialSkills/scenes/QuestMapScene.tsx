...
import { useSocialSkillsStore, getQuests, hasQuestsForGrade } from '../store/gameStore';
...
export default function QuestMapScene({ grade }: QuestMapSceneProps) {
  ...
  const handleQuestSelect = (questId: string) => {
    gameAudio.playClick();
    useSocialSkillsStore.getState().loadQuest(questId, grade);
  ...
  return (
    <div className="min-h-screen flex flex-col"
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10"
        ...
        <span className="text-sm font-bold text-pink-300">💜 Grade {grade} Social Skills</span>
        <div className="w-16" />
      </div>
